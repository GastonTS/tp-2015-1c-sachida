...
int main(int argc, char *argv[]) {
	...
	if (argc != 2) {
		printf(
				"ERROR, la sintaxis del servidor es: ./servidor.c archivo_configuracion \n");
		return -1;
	}

	getInfoConf(argv[1]);

	logger = log_create("Log.txt", "Servidor", false, LOG_LEVEL_DEBUG);
...
int escuchar() {
	...
	//este nose si va
	if (setsockopt(sock_escucha, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int))
			== -1) {
	...
	if (bind(sock_escucha, (struct sockaddr *) &my_addr,
...
int escuchar_puerto(int sock_escucha) {
	...
	if ((new_socket = accept(sock_escucha, (struct sockaddr *) &their_addr,
			&sin_size)) == -1) {
	...
	if ((mensaje.id_proceso == JOB) && (mensaje.tipo = HANDSHAKEOK)) {
	...
	if ((mensaje.id_proceso == JOB) && (mensaje.tipo = ARCHIVOS)) {
		log_info(logger, "recibi archivos");
		log_info(logger, "Mensaje del cliente: %d", mensaje.id_proceso);
		log_info(logger, "El mensaje es: %s", mensaje.mensaje);
	} else {
		log_error(logger, "No recibi Handshake OK");
	...
	int i;
	int j;
	j = list_size(mensaje.list_archivos);
	log_info(logger,"cant %d",j);
	for(i=0;i<j;i++){
			log_info(logger,"entre al for");
			cadenita = list_get(mensaje.list_archivos,i);
	...
	mensaje.tipo = MAP;
	mensaje.id_proceso = MARTA;
	strncpy(mensaje.mensaje, "MAP", sizeof("MAP"));
	memcpy(buffer, &mensaje, SIZE_MSG);

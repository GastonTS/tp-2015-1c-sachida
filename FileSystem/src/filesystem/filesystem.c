...
bool filesystem_addFile(file_t *file) {
	// Check if dir already exists. TODO, chequear contra files para evitar index error ? ..
	if (filesystem_getDirByNameInDir(file->name, file->parentId)) {
		return 0;
	}
...
bool filesystem_addDir(dir_t *dir) {
	// Check if file already exists. TODO, chequear contra files para evitar index error ? ..
	if (filesystem_getFileByNameInDir(dir->name, dir->parentId)) {
		return 0;
	}
